define(function(require, exports, module) {
  ...
  module.exports = {
    ...
    _configText() {
      $.icon_mgmt_.find('div h5.mgmt_title').text('图标广告列表');
      $.icon_mgmt_.find('div button font.mgmt_new_btn').text('新建图标广告');
      $.icon_mgmt_.find('div input.name_search').prop('placeholder', '输入广告名称');
      $.icon_mgmt_.find('div button.name_search_btn').text('搜索');
    },
    _bindUI: function() {
      // bind .name_search_btn
      $.icon_mgmt_.on("click", '.name_search_btn', function(e) {
          f_search();
        })
        // bind .name_search
      $.icon_mgmt_.on("keypress", '.name_search', function(e) {
          if (e.which == "13") f_search();
        })
        // bind .name_search val.length is 0
      $.icon_mgmt_.on('input propertychange', '.name_search', function(e) {
          if ($('.name_search').val().length == 0) f_search();
        })
        // bind .v_mnt_new_modal_btn
      $.icon_mgmt_.on("click", '.new_modal_btn', function(e) {
        newModal();
      })

      // bind grid edit
      $.icon_mgmt_.on("click", '.row_btn_edit', function(e) {
        var index = $(e.currentTarget).attr('name');
        beginEdit(index);
      })
      $.icon_mgmt_.on("click", '.row_btn_cancel', function(e) {
        var index = $(e.currentTarget).attr('name');
        cancelEdit(index);
      })
      $.icon_mgmt_.on("click", '.row_btn_end', function(e) {
        var index = $(e.currentTarget).attr('name');
        endEdit(index);
      })
      $.icon_mgmt_.on("click", '.row_btn_apply', function(e) {
        var index = $(e.currentTarget).attr('name');
  ...
  function f_initGrid() {
    ...
    g = manager = $.icon_mgmt_.find("div.listDiv").ligerGrid({
      columns: c,
  ...
  function f_getWhere() {
    ...
    var clause = function(rowdata, rowindex) {
      var key = $.icon_mgmt_.find(".name_search").val();
  ...
  function newModal() {
    var modal = BootstrapDialog.show({
      id: 'newModal',
      title: '新建图标广告',
      message: $('<div></div>').load('app/icon_mgmt_modal.html'),
