...
define(function (require, exports, module) {
    ...
    module.exports = {
        ...
        _configText() {
            $.group_mgmt_.find('div h5.mgmt_title').text('系统管理>>分组管理');
            $.group_mgmt_.find('div button font.mgmt_new_btn').text('新建分组');
            $.group_mgmt_.find('div input.name_search').prop('placeholder', '输入分组名搜索');
            $.group_mgmt_.find('div button.name_search_btn').text('搜索');
        },
        _bindUI: function () {
            // bind .name_search_btn
            $.group_mgmt_.on("click", '.name_search_btn', function (e) {
                f_search();
            })
            // bind .name_search
            $.group_mgmt_.on("keypress", '.name_search', function (e) {
                if (e.which == "13") f_search();
            })
            // bind .name_search val.length is 0
            $.group_mgmt_.on('input propertychange', '.name_search', function (e) {
                if ($('.name_search').val().length == 0) f_search();
            })
            // bind .v_mnt_new_modal_btn
            $.group_mgmt_.on("click", '.new_modal_btn', function (e) {
            ...
            $.group_mgmt_.on("click", '.row_btn_edit', function (e) {
                var index = $(e.currentTarget).attr('rowindex');
                editRow(index);
            })

            $.group_mgmt_.on("click", '.row_btn_del', function (e) {
                var index = $(e.currentTarget).attr('rowindex');
                delRow(index);
            })
    ...
    function f_initGrid() {
        ...
        g = manager = $.group_mgmt_.find("div.listDiv").ligerGrid({
            columns: c,
            onSelectRow: function (rowdata, rowindex) {
                $.group_mgmt_.find("#txtrowindex").val(rowindex);
            },
            parms: {
                source: "sys_group",
                qhstr: JSON.stringify({
                    qjson: [{groupname: $.group_mgmt_.find('.name_search').val()}],
    ...
    function newModal(title, showFun) {
        var modal = BootstrapDialog.show({
            id: 'newModal',
            title: title,
            message: $('<div></div>').load('app/group_mgmt_modal.html'),
    ...
    function newModalValidation() {
        $('#newModalForm').formValidation({
            ...
            $.post('/group/save', postParams, function (result) {
                var msg;
        ...
        $.get('/group/query', parames, function (result) {
            if (result.length > 0) {
                ...
                $.each(result, function (i, o) {
                    ...
                    if (city_name == o.city_name) {
                        initCombo1(cityid,city_name)
                    }
                })
